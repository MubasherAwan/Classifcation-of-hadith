...
df4 = pd.read_excel("C:/Users/iammu/OneDrive/Desktop/DatasetOf457AhadithForML.xlsx")
...
df5 = pd.read_excel("C:/Users/iammu/OneDrive/Desktop/DatasetOf457AhadithForML.xlsx")
...
for i in range(len(eNames)):
    ...
    narratorsNames = narratorsNames + xList
    
narratorsNames = list(dict.fromkeys(narratorsNames))
...
sparseMatrix = np.zeros((453,1107))
...
for i in range(len(sparseMatrix)):
    ...
    for j in range(len(narratorsNames)):
        if(narratorsNames[j] in dummList):
...
# Train Test Split
X_train = sparseMatrix[:370]
Y_train = np.array(y_axis[:370])
X_test = sparseMatrix[370:]
Y_test = np.array(y_axis[370:])
...
model.add(keras.layers.Dense(1150, activation="relu", input_shape=X_train.shape[1:]))
